...
4. `guidance` may only contain note ids from the GUIDANCE block. If that block is
   empty, `guidance` must be an empty list.
5. When you mention an event id in your text, write it as `EID <number>`.
6. `group_id` must be exactly the group id given below.
7. If the evidence does not support a conclusion, say so and use the assessment
